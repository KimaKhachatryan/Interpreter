...
void fill_symbol_table(std::vector<std::pair<std::string, std::list<std::string>*>>& vec)
{
	...
	vec[0].first = "typeID";
   	 ...
   	 vec[2].second->push_back("std::cout");
   	 vec[2].second->push_back("std::cin");
   	 vec[2].second->push_back("std::endl");
   	 vec[2].second->push_back("return");
   	 vec[2].second->push_back("#include");
}
	...
	for (size_t i = 1; i < vec.size(); ++i) {
		if (i == 1) {
			...
			if (vec[i] == ";") {
				...
				std::cout << "Syntax error in line " << code_line << "." <<std::endl;
				exit(0);
			}
		} else if (i == 4) {
			if (vec[i] != ";") {
...
void operations(std::vector<std::string> vec, size_t code_line)
{
	...
	if (vec[0] == "return") {
		if (vec.size() > 1) {
			if (vec[1] == ";") {
				std::cout << "Error in line " << code_line << ".\n"
						  << "Non void function doesn't return value!" << std::endl;
				exit(0);
			}
		} 
	...
	} else if (vec[0] == "if" || vec[0] == "while") {
		if (vec.size() == 3) {
			if (vec[1][0] != '(' || vec[1][vec[1].size() - 1] != ')') {
				error.message(17);
				exit(0);
			}

			if (vec[2] != ";") {
				error.message(17);				// expected (condition) {
				exit(0);
			}
		} else {
			error.message(0);					//syntax error
			exit(0);
		}

	} else if (vec[0] == "std::cout" || vec[0] == "std::cin") {
		std::string lib = "<iostream>";
		if (!is_contains(lib)) {
			std::cout << "Error! \niostream library does not included." << std::endl;
			exit(0);
		}
		...
		if (vec.size() >= 4) {
			if (vec[0] == "std::cout") {
				if (vec[1] != "<<") {
					error.message(18);					//expected << 
					exit(0);
				}

				if (!name_lookup(vec[2])) {
					if (vec[2][0] != '"' || vec[2][vec[2].size() - 1] != '"') {
						error.message(12);				//variable not found
						exit(0);
					}
				}
				...
				if (vec[3] != ";") {
					if (vec[3] != "<<") {
						...
						if (vec.size() == 6) {
							...
							error.message(20);					//expected primary expresion befor ;
							exit(0);
						}
					}
				}
...
*/
	...
	} else if (vec[0] == "#include") {
		if (vec.size() == 2) {
			if (vec[1] == "<iostream>") {
				return;
			} else {
				std::cout << "Error in line " << code_line << ": File not found!" << std::endl;
				exit(0);
			}
		} else {
			error.message(0);					//syntax error
...
{
	auto i_it = integer.find(str);
	if (i_it != integer.end()) {
		return true;
	}

	auto d_it = doubl.find(str);
	if (d_it != doubl.end()) {
		return true;
	}
...
T find_val(std::string str)
{
	auto i_it = integer.find(str);
	if (i_it != integer.end()) {
		return static_cast<T>(i_it->second);
	}

	auto d_it = doubl.find(str);
	return static_cast<T> (d_it->second);
}
